...
class Recommender:
    ...
    def __init__(self, name, trainable=True, verbose=False, **kwargs):
        self.name = name
        ...
        ## for limit model ranking within a given article pool
        self.article_pool = None

        for key, value in kwargs.items():
            setattr(self, key, value)  # Creates attributes dynamically
        
        # print(f"article_pool:{self.article_pool}")

        # for saving ranked list and prediction scores
        self.ranked_items = {}
        self.item_scores = {}

        ## optional
        # default_saving_option = False
        # if self.article_pool is not None: 
        #     default_saving_option = True
        # self.output_item_ids = kwargs.get("output_item_ids", default_saving_option)
        self.item_scores_mapped_indices = {}
    ...
    def save_recommendations(self, save_dir):
        ...
        scores_file_path = os.path.join(save_dir, "item_scores.pkl")

        scores_mapped_index_path = os.path.join(save_dir, "item_scores_mapped_indices.pkl")

        # Save the recommendation dictionary using pickle
        with open(ranked_items_file_path, 'wb') as f:
            pickle.dump(self.ranked_items, f)

        # print(f"saved files self.ranked_items:{self.ranked_items}")
        print(f"Recommendations saved to {ranked_items_file_path}")
        # Save the scores (user_id -> item scores) to a separate file
        with open(scores_file_path, 'wb') as f:
            pickle.dump(self.item_scores, f)
        print(f"Item scores saved to {scores_file_path}")
        
        with open(scores_mapped_index_path, 'wb') as f:
            pickle.dump(self.item_scores_mapped_indices, f)

        print(f"Item scores mapped index saved to {scores_mapped_index_path}")
    ...
    def rank(self, user_idx, item_indices=None, k=-1, **kwargs):
        ...
        if self.article_pool is not None:
            ...
            ## the item prediction scores output doesn't correspond to the item orders in the cornac
            # indexing system, so by default, the index order need to be stored. 
        # List example
        # self.article_pool = ['article2, 'article1', 'article3']
        ...
        # # NumPy array example
        # import numpy as np
        # self.article_pool = np.array(['article2', 'article1', 'article3'])
            ...
            assert isinstance(self.article_pool, (list, set, tuple, np.ndarray)), \
        "article_pool must be a list, set, tuple, or numpy array"
            
            assert len(self.article_pool) > 0, "article_pool must not be empty"

            if isinstance(self.article_pool, np.ndarray):
                article_pool = self.article_pool.flatten()
            else:
                article_pool = self.article_pool


            impression_items_list = [
                item_id2idx[iid] for iid in article_pool if iid in item_id2idx
            ] # cornac item id of the articles in the article pool

            assert len(impression_items_list) > 0, (
                "article_pool must contain at least one valid item ID present in item_id2idx"
            )


            test_impression_items = np.asarray(impression_items_list)

            ranked_items, item_scores = self.rank_partial( user_idx=user_idx, item_indices = test_impression_items, item_idx2id = item_idx2id, user_idx2id = user_idx2id)

            self.ranked_items[user_idx] = ranked_items

            self.item_scores[user_idx] = item_scores

            # if self.output_item_ids:
            self.item_scores_mapped_indices[user_idx] = test_impression_items
    ...
    ## added for ranking only the items in the user-specified article pool
    def rank_partial(self, user_idx, item_indices=None, **kwargs):
        ...
        ### fiter item_indices and check if they are in the given item indices

        # item_idx2id = kwargs.get("item_idx2id", None)
        # user_idx2id = kwargs.get("user_idx2id", None)
        # item_idx2id = {v: k for k, v in self.iid_map.items()} # cornac item ID : raw item ID
        # user_idx2id = {v: k for k, v in self.uid_map.items()} # cornac user ID : raw user ID
        known_item_scores = self.score(user_idx, item_indices, **kwargs)
        # known_item_scores = self.score(user_idx, item_indices,item_idx2id = item_idx2id, user_idx2id =user_idx2id )
        ...
        # print(f"user {user_idx}: recommendation:{ranked_items[:20]}")
        return ranked_items, item_scores
