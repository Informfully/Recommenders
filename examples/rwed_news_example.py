...
# DESCRIPTION:  Example for random walk(RWE-D) model experiment in an example dataset                               #
#############################################################################################################

import cornac
from cornac.datasets import mind as mind
from cornac.eval_methods import BaseMethod
from cornac.models import RWE_D
...
# initialize models
model_without_article_pool =  RWE_D(name = "RWED") #example without article pool

model_with_article_pool =  RWE_D(name = "RWED_articlePool",article_pool = impression_iid_list) #example with article pool

# define metrics to evaluate the models
metrics = [ NDCG(k=20), Recall(k=20)]


output_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.', 'output_rwed_result')
