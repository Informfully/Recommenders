###### Example Script: NPA Model ######
# This script demonstrates the standard experiment of the NPA model.
#
# 1. Data Preparation:
...
#
# 2. NPA Model Configuration:
#    - The NPA model utilizes user history and impression logs for training.
...
#    - Combines the NPA model, metrics, and rerankers in a Cornac `Experiment`.
...
def main():
    ...
    # Set up the NPA model
    model_without_article_pool = NPA(name= "NPA",
    ...
    model_with_article_pool = NPA( name= "NPA_articlePool" ,
        wordEmb_file=word_embedding_path,
        wordDict_file=word_dict_path,
        newsTitle_file=title_dict_path,
        userHistory=user_item_history,
    ...
    Target_Mind_distribution = {
            "sentiment": {"type": "continuous", "distr": [
                ...
                {"description": "composition", "contain": [["party1","party4"], ["party3", "party6"]], "prob": 0.2},
    ...
    ## Static re-ranking is set without article Pool
    sentiment_party_greedy_reranker = GreedyKLReranker(name = "GreedyKL_noPool",
    ...
    sentiment_party_pm2_reranker = PM2Reranker(name = "PM2_noPool",
        item_dataframe= article_feature_dataframe_keep_no_party,
        diversity_dimension=["sentiment", "entities"],
        top_k=targetSize,
        target_distributions = Target_Mind_distribution,
    ...
    sentiment_party_mmr_reranker = MMR_ReRanker(name = "MMR_noPool",
                                                top_k=targetSize,
    ...
    ## Dynamic re-ranking is set with article Pool
    ...
    output_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.', 'output_npa_reranking_result')
